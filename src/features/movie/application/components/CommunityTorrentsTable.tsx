...
export const CommunityTorrentsTable: React.FC<CommunityTorrentTableProps> = ({
  items,
  loading,
}) => {
  ...
  const RowItem = (item: import("../../domain/entities/Hashes").HashResult) => {
    ...
    return (
      <Table.Row key={item.hash} id={item.hash}>
        <Table.Cell>{item.name.replace(/\./g, " ")}</Table.Cell>
        <Table.Cell className="flex items-center gap-2">
          <Button variant="ghost" onClick={handleClick}>
            <PlayIcon />
            Ver
          </Button>
          <CopyTorrentButton title={item.name} torrent={torrent} />
          <OpenTorrentButton title={item.name} torrent={torrent} />
        </Table.Cell>
      </Table.Row>
    );
  };
              ...
              <Table.Body
                ...
                renderEmptyState={() => (
                  <NoDownloadsAvailable
                    message={loading ? "Cargando..." : undefined}
                  />
                )}
              >
